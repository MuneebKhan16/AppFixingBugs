...
const EventProfile = () => {
  const {UserPost} = useContext(eventContext);
  ...
  const Delete_Event = async item => {
    ...
    if (data.status == 1) {
      NavService.navigate('TabComp', {screen: 'Home'});
    }
  };
  const Event_data = async () => {
    const events = await show_eventCreater_event(userProfile?.id);
      ...
      /> */}
        <View style={styles.content}>
          <View style={styles.container}>
            <View>
              <Text style={styles.hd1}>Joining Date</Text>
            </View>
            <View style={{marginRight: 15}}>
              <Text style={styles.txt}>
                {DateReadbleFunction(userData?.created_at || 'now')}
              </Text>
          ...
          <View
            ...
            <Text style={styles.hd2}>No. of Events</Text>
            <Text style={styles.hd3}>No. of Attendance</Text>
          </View>
          <View
            style={{
              flexDirection: 'row',
